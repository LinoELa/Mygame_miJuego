...
#2put that on a surface 
#3display that surface
...
def display_score():
    ...
    score_surf = text_font.render(f'Score : {current_time}' , False , (64,64,64))  #f '{ ... } lo convertimos a string  #TIEMPO DEL JUEGO 
...
titulo = pygame.display.set_caption('Pygame Game ')
...
#JUEGO QUE EMPIECE Y TERMINE 
        if Game_Active: #CHEKEAR PARA QUE FUNCIONE CUANDO EL JUEGO ESTA ACTIVO
            ...
            if event.type == pygame.KEYDOWN and event.key == pygame.K_SPACE: 
                Game_Active = True
                snail_rect.left = 800
                start_time = int( pygame.time.get_ticks() / 1000 ) #PARA QUE EL TIEMPO TAMBIEN SE REINICIE AL REINIICIAL EL JUEGO
    ...
    if Game_Active: #funciona mietras el juego esta activo
        ...
        # ventana.blit(score_surf,score_rect)
        #       
        display_score() #LA FUNCION DEL SCORE TIEMPO
